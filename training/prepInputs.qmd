...
`postProcess` will be triggered if the user passes any of the arguments
of `postProcessTo` (`to`, `maskTo`, `cropTo`) `studyArea` or
`rasterToMatch`.
...
`prepInputs` is friendly with `Cache`. In addition to the local stashing
of a file, the GIS itself may take a long time. We can just `|> Cache()`
at the end and we have a workflow that doesn't need to be broken.
