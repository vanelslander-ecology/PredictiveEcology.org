...
From here onward, we will need to manually open the module code
file. Every SpaDES module is defined by having at least 1 file that is
named with `<modulePath>/<moduleName>/<moduleName>.R`. Even though
we can do a lot with `newModule()`, we will need to get used to
