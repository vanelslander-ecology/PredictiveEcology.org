...
We use the terms `expectsInput` and `createsOutput` to describe the inputs
and outputs in the metadata. This makes it clear that the metadata do *not* specify
which they will *go get*; rather, it makes it clear that it doesn't matter *where*
the inputs are coming from. They could come from one of three sources: 
a user, another module, or defaults that the developer sets up. Likewise, the
module specifies which outputs it creates, without specifying "for what other module".
...
Next, *what are the package dependencies?*. We call this `reqdPkgs` in the
SpaDES metadtaa. We see that there are three
...
is. **FROM HERE, OPEN THE FILE**, e.g., by copy-paste the file path
(pick the `.R` file NOT the `.Rmd` file for now)\*\*

::: {.callout-note appearance="simple" icon="false"}
**Opening a module file**

From here onward, we will need to actually manually open the module code
file. Every SpaDES module is defined by having at least 1 file that is
named with the `<modulePath>/<moduleName>/<moduleName>.R`. Even though
we can do a lot from the `newModule`, we will need to get used to
opening, examining and changing the code in the module code file.
:::
...
-   **return(sim)** -- must always be present. It is by default, but
    don't delete it!
-   **don't need to run this function** -- SpaDES runs it when it is
    time.
:::
...
`createsOutput` of `model`. But this new `visuzliation` module will
...
following, you will get an error, unlink in the previous chapter where
