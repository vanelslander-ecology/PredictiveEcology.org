...
is. **FROM HERE, OPEN THE FILE**, e.g., by copy-paste the file path (pick the `.R` file NOT the
`.Rmd` file for now)**

::: {.callout-note appearance="simple" icon="false"}
**Opening a module file**

From here onward, we will need to actually manually open the module code file.
Every SpaDES module is defined by having at least 1 file that is named with 
the `<modulePath>/<moduleName>/<moduleName>.R`. Even though we can do a lot from
the `newModule`, we will need to get used to opening, examining and changing 
the code in the module code file.
:::
...
following, you will get an error, unlink in the previous chapter where we
had `x` defined in the `init` event. 
