...
-   limit the number of objects in `.GlobalEnv`. Instead, put them in
    functions or inside a module. When objects are in modules, you can 
    add default object values (e.g. via `.inputObjects()`), or save them 
    into files that are loaded by `setupProject` or `simInit`. 
    Alternatively, you can pass objects or code to make them directly to 
    `setupProject()` as `...` arguments, or to the `simInit` `objects` 
    arguments. See @sec-moduleinputs and, for an example
    with `setupProject`, @sec-LandRExample.
