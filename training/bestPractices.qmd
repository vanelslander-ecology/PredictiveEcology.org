...
-   limit the number of objects in `.GlobalEnv`. Instead, put them in 
    functions -- such as with `setupProject` as arguments -- or inside a module. 
    When objects are in modules, you can add default object
    values (e.g. via `.inputObjects`), or save them into
    files that are loaded by `setupProject` or `simInit`. See
