...
The name `simList` has the word "list" in it, but it is actually an `environment`
```{r,eval=FALSE}
sim <- simInit()
is(sim, "environment")
# [1] TRUE
...
If you don't know what an `environment` is in `R`, it is OK. The most important
feature of an `environment` is that is it `pass-by-reference`, which means it is
not copied when it is "assigned" somewhere. This makes `SpaDES` fast enough to use
for huge workflows.
...
- using [`simInit()`](https://rdrr.io/cran/SpaDES.core/man/simInit.html)
