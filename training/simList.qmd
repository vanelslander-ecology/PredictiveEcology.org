...
important feature of an `environment` is that is it `pass-by-reference`,
which means it is not copied when it is "assigned" somewhere. This makes
`SpaDES` fast enough to use for huge workflows.
