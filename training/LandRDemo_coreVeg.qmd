...
`googledrive` R package (part of the `tidyverse`
family)[^landrdemo_coreveg-1].
...
cache, this time around[^landrdemo_coreveg-2].
...
-   Generate 10 *independent* simulations and run them in sequence
    (@lst-exp) or in parallel (@lst-expparall), using the `future`
    package.
...
We pass all raster objects from the saved files directly, to ensure they are 
linked to these file paths explicitly.
...
outValid <- setupProject(
  ...
  simulationOutputs = {
    repDirs <- list.files(SpaDES.core::outputPath(simOut), "_rep", full.names = TRUE) 
    cohortDataFiles <- sapply(repDirs, list.files, pattern = "cohortData", simplify = FALSE) 
    cohortDataDT <- sapply(cohortDataFiles, as.data.table, simplify = FALSE) |>
      rbindlist(idcol = "dir")
    cohortDataDT[, objectName := "cohortData"]
    cohortDataDT[, saveTime := as.integer(sub(".*([[:digit:]]{4}).*", "\\1", V1))]
    cohortDataDT[, file := file.path(dir, V1)]
    cohortDataDT[, `:=`(dir = NULL, V1 = NULL)]
    
    PGMapFiles <- sapply(repDirs, list.files, pattern = "pixelGroupMap", simplify = FALSE) 
    PGMapDT <- sapply(PGMapFiles, as.data.table, simplify = FALSE) |>
      rbindlist(idcol = "dir")
    PGMapDT[, objectName := "pixelGroupMap"]
    PGMapDT[, saveTime := as.integer(sub(".*([[:digit:]]{4}).*", "\\1", V1))]
    PGMapDT[, file := file.path(dir, V1)]
    PGMapDT[, `:=`(dir = NULL, V1 = NULL)]
    
    outDT <- rbind(cohortDataDT, PGMapDT)
    outDT
  },
...
[^landrdemo_coreveg-1]: See
    [`?googledrive::drive_auth`](https://rdrr.io/cran/googledrive/man/drive_auth.html)
    for more information.

[^landrdemo_coreveg-2]: See @sec-caching about caching.

[^landrdemo_coreveg-3]: By default, *Biomass_borealDataPrep* tries to
