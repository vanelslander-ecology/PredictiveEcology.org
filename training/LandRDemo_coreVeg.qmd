...
live plotting in *Biomass_core*.

Note that because `simOut` is actually a copy of `simInitOut` (see the
output of `SpaDES.core::completed(simInitOut)`), this means that
`simInitOut` has to be re-generated for this purpose, otherwise `spades`
will try to resume the simulation from the last year.

Thanks to `Cache()`, it will only take seconds to "reset" `simInitOut`.
You will also notice that `init` events are retrieved from cache, this
time around.
