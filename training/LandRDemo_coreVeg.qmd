...
googledrive::drive_auth(email = "eliotmcintire@gmail.com", cache = "c:/Eliot/SpaDES_book/googledrive_auth_cache")
```
...
if (packageVersion("SpaDES.project") < "0.1.1")
...
out <- setupProject(
  ...
  # sideEffects are anything that needs doing, but doesn't return a value
  sideEffects = {
    if (interactive()) {
  ...
  # sppEquiv is a table that defines equivalent names for species e.g., Pice_mar and Pice_Mar are identical
  sppEquiv = {
