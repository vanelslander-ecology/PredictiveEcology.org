...
**6 GB RAM -- estimated RAM for executing**
...
"accessorts", i.e., "access something from the simList".  `completed`
shows the events that were run. `elapsedTime` shows how much time 
each event too.
...
Setting `.plots = "png"` will turn on all known plotting (that uses
the `Plots` function and save them all as `png` files in the `figures` subfolder.
