...
**1. Do I need to repeat this?**

**2. If yes, how often and/or when?**

Once we answer these, we can use the function `scheduleEvents` to set them up
in whatever sequence we want.
:::
...
Now we need to tell SpaDES when to do and when to iterate each of the
steps or "events". The `Init` we do once, at the start. The `plotFn` we
do once, but this time it is at the "end". The `.inputObjects` always
...
We add code to make a new event, called `"predictions"` (the name must
match what we wrote in the `scheduleEvent` above). In this event, we
"do" the prediction, then we schedule the next prediction at "time plus
1"
...
::: {.callout-tip}
## Time in `SpaDES`
It is useful to understand `time` in SpaDES. Time has units (e.g., "year") which
is defined by the modules metadata `timeunit`. So, if `time(sim)` is `1` or `2024`,
that would be in `year` units, if the module is in `year`. To know what "time" it is
in a module, the easiest function is `time(sim)`. That will give you the "time". 

Time, in SpaDES, is a way of ordering events. If one event happens at time 2024,
and another event happens at 2025, then the 2024 event will happen first.

The other two useful functions are `start(sim)` and `end(sim)`, which will give
you the starting time of the `spades` call and ending time respectfully. These
would have been specified in `simInit`.
:::
...
```
This would schedule 3 *predictions*, during the next three "time steps". This may be 
very clear, but it would be difficult to do this for dozens of years (could be done
with a for-loop).

A "bottom up" approach is to schedule the *predictions* event once, then each time
that *predictions* event occurs, reschedule it.
...
```
Each time the *predictions* event happens, it will reschedule itself forever! Very simple.
The workflow sequence emerges from the self-scheduling.
:::
...
yet at that time. So, we will choose to schedule `plotFn` to run at `end(sim)`:
...
Like in @sec-moduleFilesAndMetadata, we have to update the module metadata... what
