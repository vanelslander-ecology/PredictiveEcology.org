...
[`SpaDES.project::setupProject()`](https://spades-project.predictiveecology.org/reference/setupProject.html) deals with a number of challenges that
crop up with working with multiple modules created by potentially many
people.
...
Note how it downloads the listed modules directly from their GitHub repositories 
(e.g. [tati-micheletti/speciesAbundance](https://github.com/tati-micheletti/speciesAbundance))
...
The output of `setupProject` is a list of argument values that can be passed to
`simInit` or `simInitAndSpaDES`. The latter will not only initialise the workflow
but also run it.
