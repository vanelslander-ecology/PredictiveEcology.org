...
`reproducible::prepInputs` that help deal with module inputs ([Inputs Tip @tip-Inputs]).
...
Because parameters are usually module-specific, and need to be passed to with reference
to a module. "Global" parameters are the exception to this rule:
...
We then include the following in the module metadata section
...
2. if an object is missing, [it may make sense to fill in the
...
3. an object called `studyArea`. [The developer tests whether it
