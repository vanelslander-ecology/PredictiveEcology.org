...
The **inputs** of a SpaDES module can be any R object and should be specified in the `inputObjects` section of the metadata if it is needed for the module to run. These should be used in the `sim` using e.g., `sim$` somewhere in the module. 
:::
...
A user can pass **objects** into the `simInit` and these will be put into the `simList`
...
See [`?SpaDES.core::simInit`](https://reproducible.predictiveecology.org/reference/suppliedElsewhere.html).
...
This works for only the simplest cases. In the case of a module, objects can be arbitrarily complicated and so this approach doesn't work well. Instead, there is a function that is called every time a module is initialized where a developer can put anything they want to specify defaults. It is generally most useful to use `if (!SpaDES.core::suppliedElsewhere())` i.e., if the object isn't there, then give it a value.
...
We will leave this section for a subsequent chapter when we are working with multiple modules.

## Questions

1. In `.inputObjects`, why don't we just write `if (!is.null(sim$y))`? See [`?SpaDES.core::suppliedElsewhere`](https://reproducible.predictiveecology.org/reference/suppliedElsewhere.html)
