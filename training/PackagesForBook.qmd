...
`SpaDES.project` and its dependencies, then uses
`setupProject` to download and install a list of R packages hosted on GitHub and
a single SpaDES module (*Biomass-core*) which has over 130 package dependencies.
...
Note that `setupProject` will change your working directory to `./Introduction`.
If that is not where you want to keep working, restart R and make sure you set 
your working directory to where you want it to be (if restarting is not sufficient).
...
installed in a folder associated with the project "Introduction"
