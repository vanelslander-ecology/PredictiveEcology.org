...
Looking at this code above, we can imagine that the first 3 lines are conceptually 
different -- *fit a dummy model* -- from the last line -- *visualize it*. 
Lets put these into 2 "modules", and then run them in sequence.

## Create a module

We use the `SpaDES.core` function `newModule()` to make a new module.

Step 1 -- move the code to an "init" event. This "init" even is a
...
We will explain this in much more detail later. Here, we want to keep the `model` 
that we fit. But we don't care about the `x` and `y`. So we assign the `model` to `sim`. 
...
code (like loading a library or sourcing a function) then `spades` which
executes it.
...
Our next step is to divide the pieces into conceptual chunks. Let's now create a 
new module for the `plot` ... a "visualization module".
...
And run both together. We name them both in the `module` argument. We are using `simInitAndSpades` 
which is a shortcut to running the two function separately.
```{r,eval=FALSE}
...
Next, we will explore a key feature of SpaDES modules: the metadata.
