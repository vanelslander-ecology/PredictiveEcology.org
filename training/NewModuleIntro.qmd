...
*visualize it*. Lets put these into 2 "modules", and then run them in
sequence.
...
And run both together. We name them both in the `module` argument. We
are using `simInitAndSpades` which is a shortcut to running the two
function separately.
