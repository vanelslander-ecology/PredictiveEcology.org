...
`setupFunctions()` respected the folder structure in `PredictiveEcology.org` 
and placed these files in `tutos/castorExample/` *within* the specified
`projectPath`. We'll need to remember this for later.
...
Note that all folder/file paths are *relative* to `projectPath`, even the file 
path passed to `parameters` (remember the file path from above?).
