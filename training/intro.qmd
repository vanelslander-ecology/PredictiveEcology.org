...
[![Functions and modules as key tools to achieve workflow nimbleness. From @mcintire2022](assets/img/PERFICTfig1.jpg)](https://onlinelibrary.wiley.com/doi/10.1111/ele.13994)
...
-   **SpaDES has "embedded" metadata** – or at least it makes the
    developer think about it[^2]. Each module is defined by listing not
    only the inputs and outputs, but also their descriptions and types
    of object class, the module's description, its operating time scale,
    package and module dependencies, and much more. Some of the metadata
    is essential to chain modules (lists of inputs and outputs), some is
    not but can greatly facilitate workflow setup (e.g. declaring module
    dependencies), while other metadata is simply good to have (e.g. a
