...
-   **SpaDES has "embedded" metadata** – or at least it makes the
    developer think about it[^intro-2]. Each module is defined by not
    only listing the inputs and outputs, but also their descriptions and
...
Finally, both `targets` and SpaDES have embedded caching mechanisms that
in SpaDES's case can be used and controlled at several levels (by the
user, in the module code, etc.) – see @sec-caching on SpaDES caching
mechanisms.
