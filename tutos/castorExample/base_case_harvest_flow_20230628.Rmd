---
title: "STSM-Castor comparison: base case harvest flow; determined by STSM adn fit to Castor model."
author: "Tyler Muhly"
date: "28 June 2023"
output:
html_document: 
...
This scenario was developed to compare a 'base-case' STSM-SELES timber
supply model to the Castor timber supply model. Both of these models are
deterministic, spatial, simulation models. The same datasets and
parameters were used in each model to assess whether they determine
similar spatial timber supply outcomes.

## Scenario

This scenario replicates the 'base-case' harvest flow that is estimated
following the STSM methods, where (in summary), the max even harvest
flow is determined, short term harvest is increased if possible, to the
point where it has no effect on the mid-term harvest flow (i.e., drops
below the max even harvest flow), harvest flow cannot deceased more than
10% from one decade to the next, and long-term harvest level is
increased to teh extent possible The harvest priority queue is 'oldest
first' and includes blocking and roading but no constraints.
...
scenario = data.table (name = "stsm_base_case",
                       description = "Priority queue = oldest first. Adjacency constraint = None. Includes roads (mst) and blocks (pre). Harvest flow = 147,300 m3/year in decade 1, 133,500 m3/year in decade 2, 132,300 m3/year in decades 3 to 14 and 135,400 m3/year in decades 15 to 25. Minimum harvest age = 80 and minimum harvest volume = 150")
